...
//上傳圖片
var uploadimg = function uploadimg(iurl, idata, ipic, irimg, ifile) {
...
function checkOldGreenCard(id) {
  ...
  for (a = 1; a <= 6; a++) {
    ...
    if (k == -1) return false; //myArr[a]=id.charAt(a);
  }

  return true;
}

function checkForeigner(id) {
  //西元年月日+英文名前兩碼
...
function checkbirthday(dyear, dmonth, dday) {
  //判斷20-70歲之間
...
function checkbirthday2(older, dyear, dmonth, dday) {
  //判斷幾歲
...
function reloadCAPTCHA(idname) {
  //重新載入驗證碼圖片
  $(idname).attr("src", "CAPTCHA/CAPTCHA_image.asp?" + Date());
}

function showhide(odiv, sts) {
  //顯示或隱藏
  var vobj = $(odiv);
